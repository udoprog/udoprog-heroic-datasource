...
System.register(['./utils'], function(exports_1) {
    ...
    function HeroicDatasource(instanceSettings, $q, backendSrv, templateSrv) {
        ...
        this.query = function (options) {
            ...
            return this.doRequest('/query/batch', { method: 'POST', data: data }).then(function (data) {
                ...
                for (var k in results) {
                    ...
                    for (var i = 0, l = groups.length; i < l; i++) {
                        var group = groups[i];
                        var name;
                        var scoped = _this.buildScoped(group, result.common);
        ...
        this.buildTags = function (tags) {
            var parts = [];
            for (var k in tags) {
                parts.push(utils_1.quoteString(k) + ": " + utils_1.quoteString(tags[k]));
            }
            return "{" + parts.join(", ") + "}";
        };
        this.seriesCount = function (filter) {
        ...
        this.doRequest = function (path, options) {
            options = options || {};
    ...
    exports_1("HeroicDatasource", HeroicDatasource);
    return {
        setters:[
            function (utils_1_1) {
                utils_1 = utils_1_1;
